    ...
    >
      <div>
        <div className='relative'>
          ...
          <span className={`absolute b-0 l-0 h-1 bg-blue-500 ${loading ? 'w-full transition-all duration-[10s]' : 'w-0'}`}></span>
        </div>
      ...
      {!isImageLoaded &&
        <div className='flex w-full max-w-xl bg-neutral-500 text-white text-sm  mt-10 rounded-full'>
          ...
          <button type='submit'
            className='bg-zinc-900 px-12 sm:px-16 py-3 rounded-full text-white'>Generate</button>
        </div>
      }

      {isImageLoaded &&
        <div className='flex gap-4 mt-10 flex-wrap justify-center text-white text-sm p-0.5 rounded-full'>
